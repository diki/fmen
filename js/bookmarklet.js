...
function bookmark(url, isVideo) {
    ...
    var documentUrl = 'http://'+domain+'/men?image_url='+urlencode(url)+'&source_url='+urlencode(location.href);
    ...
    var pinpornWindow = window.open(documentUrl,'name','width=900,height=650');
    if (window.focus) {
        pinpornWindow.focus();
    }
...
if (allowPin) {
    ...
    if (images.length == 0) {
        alert("No image found in this document");
    } else {
        createOverlay(images);
    }
}